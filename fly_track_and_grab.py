...
外部系统控制命令:
  - http://192.168.42.15/lower → LOWER (下降)
  - http://192.168.42.15/pull → PULL (拉起)
  - http://192.168.42.15/stop → STOP (停止)

使用方法:
  python fly_track_and_grab.py --classes person
  python fly_track_and_grab.py --classes keyboard --stable-time 5 --wait-time 5 --pull-time 3
"""
...
class ExternalSystemController:
    ...
    def __init__(self, base_url: str = "http://192.168.42.15", timeout: float = 5.0):
    ...
    def send_command(self, command: str) -> bool:
        ...
        try:
            print(f"[EXTERNAL] Sending command: {command.upper()} -> {url}")
            response = requests.get(url, timeout=self.timeout)
            
            if response.status_code == 200:
                print(f"[EXTERNAL] ✓ {command.upper()} success: {response.text.strip()}")
                return True
            else:
                print(f"[EXTERNAL] ✗ {command.upper()} failed: HTTP {response.status_code}")
                return False
                
        except requests.exceptions.Timeout:
            print(f"[EXTERNAL] ✗ {command.upper()} timeout ({self.timeout}s)")
            return False
        except requests.exceptions.ConnectionError:
            print(f"[EXTERNAL] ✗ {command.upper()} connection error (is {self.base_url} reachable?)")
            return False
...
使用示例:
  ...
  # 自定义时间参数
  python fly_track_and_grab.py --classes chair \
  ...
  # 超保守设置 (防止过冲)
  python fly_track_and_grab.py --classes person --kp 3 --max-speed 3
  
  # 自动起飞 + 自动降落
  python fly_track_and_grab.py --classes person --takeoff-on-start --auto-land-on-exit
...
参数调优:
  ...
  问题: 跟踪太慢?
    ...
    # 外部系统
    p.add_argument("--system-url", default="http://192.168.42.15",
                   help="External system base URL")
...
class FlyTrackAndGrab:
    ...
    def __init__(self, args):
        ...
        self.detector = None
        self.tracker = None
        self.external_system = ExternalSystemController(args.system_url)
    ...
    def run(self):
        ...
        print()
        print("=" * 75)
        print("   TARGET TRACKING + EXTERNAL SYSTEM CONTROL WITH SAFETY SYSTEM")
        print("=" * 75)
        print()
        print(f"  Tracking: {self.args.classes}")
        print(f"  External system: {self.args.system_url}")
